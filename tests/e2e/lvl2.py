...
def get_data_files_names():
        dir_path = str(Path.cwd()) + "/../tests/e2e/data_lvl2/"
...
def gen_data(file_name):
        n_matrixes = parse_data_file(file_name)
        ...
        for i in range(n_matrixes):
                matrixes.append(gen_matrix(matrixes_sizes[i], matrixes_sizes[i + 1]))
                n_elems += matrixes_sizes[i] * matrixes_sizes[i + 1]

        data = [str(n_matrixes * 2 + n_elems)]

        for matrix, i in zip(matrixes, range(n_matrixes)):
                data.append("\n")
                data.append(str(matrixes_sizes[i]))
                data.append(str(matrixes_sizes[i + 1]))

                for x in matrix:
                        for y in x:
                                data.append(str(y))

        return data     # n_elems ...elems...
